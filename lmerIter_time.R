...
isplineMLE <- function(y, x, df, degree, isMat) {
  ...
  fit <- constrOptim(theta = startbeta, f = neloglh, y = y, basis = basis,
                     grad = NULL, ui = basis, ci = rep(0, nrow(basis)),
...
##' error variance change with time
iter <- function(formula, data = NULL, time, df, degree, tolerance = 1e-4) {
  ...
  isMat <- iSpline(c(min(time), max(time)), 
                   df = df, degree = degree, intercept = TRUE)
  
  for (i in 1:10) {
    sfit <- isplineMLE(fiterr, time, df = df, degree = degree, isMat = isMat)
    basis <- predict(isMat, time)
    ...
    fiterr <- resid(mixmod.1)
  }
  
  print(paste0('iteration times:', i))
...
errVar <- function(object, degree, time){
  X <- getME(object$mixmod.1, name = "X")
  df <- length(object$sfit$par) - 1
  
  isMat <- iSpline(c(min(time), max(time)), df = df, degree = degree, intercept = TRUE)
  basis <- predict(isMat, time)
...
##' select df
fitSelect <- function(formula, data = NULL, time, df, degree, tolerance = 1e-4) {
  df.n <- length(df)
  aic.n <- rep(Inf, df.n)
  bic.n <- rep(Inf, df.n)
  df.aic <- rep(0, df.n)
  ...
  count <- 0
  for (i in 1:df.n) {
    t <- try(iter(formula=formula, data=data, time = time, df=df[i], degree=degree,
    ...
    if (is.list(t)) {
      ...
      covE <- diag(as.vector(errVar(t, degree = degree, time = time)))
...
##' input iter object
bootsd.para <- function(formula, object, data, time, response, ID, degree, 
                        tolerance = 1e-4, nsim) {
  X <- getME(object$mixmod.1, name = "X")
  ...
  if (nsim > 1) {
    for (i in 1:nsim) {
      ...
      t <- try(iter(formula = formula, data = data.boot, time = time, df = df, 
...
lmerIter <- function(formula, response, ID, data = NULL, time, df, degree, nsim,
                     tolerance = 1e-4) {
  
  fit <- fitSelect(formula=formula, data=data, time = time, df=df, degree=degree, 
                   tolerance = tolerance)
  ...
  if (is.list(fit)) {
    if (fit$df.aic == fit$df.bic) {
      sd.paraboot <- bootsd.para(formula = formula, object = fit$model.aic, 
                                 data = data, time = time, response = response, ID = ID, 
      ...
      AICsd.paraboot <- bootsd.para(formula = formula, object = fit$model.aic, 
                                    data = data, time = time, response = response, ID,
                                    degree = degree, tolerance = tolerance, nsim = nsim)
...
##' power function: var(error) = sdE^2*(fixed)^(2power)
...
lmIter <- function(formula, data, df, degree, tolerance = 1e-8, nsim) {
